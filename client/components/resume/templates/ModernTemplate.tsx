          ...
          resume.projects.map((project, index) => (
            <div key={index} className="mb-6">
              ...
              {project.technologies?.length > 0 && (
                <p
                  className={`mt-1 text-sm font-medium ${theme.secondary}`}
                >
        ...
        {resume.languages?.length > 0 ? (
          <div className="flex flex-wrap gap-3">
            {resume.languages.map((language, index) => (
              <div
                key={index}
                className={`rounded-full border px-4 py-2 ${theme.border} ${theme.light}`}
              >
                <span
                  className={`font-medium ${theme.primary}`}
                >
                  {language.name}
                </span>
      ...
      {/* ================= ACHIEVEMENTS ================= */}
        ...
        {resume.achievements?.length > 0 ? (
          resume.achievements.map((achievement, index) => (
            <div
              key={index}
              className={`mb-5 rounded-lg border p-4 ${theme.border} ${theme.light}`}
            >
              <h3 className="text-lg font-semibold text-gray-900">
                {achievement.title}
              </h3>
